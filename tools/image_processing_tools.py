...
class ImageProcessingTools:
    ...
    def composite_images(
        ...
        Pure compositing operation without decision-making logic.
        
        Args:
            base_image_path: Path to base image
            overlay_image_path: Path to overlay image
            position: Position to place overlay (x, y)
            blend_mode: Blending mode (normal, multiply, screen, etc.)
            
        Returns:
            Compositing operation results
        """
        try:
                ...
                with Image.open(overlay_image_path) as overlay_img:
                    ...
                    if blend_mode == "normal":
                        ...
                        if overlay_img.mode == "RGBA":
                            ...
                            result_img.paste(overlay_img, position)
                        ...
                        temp_img = Image.new("RGBA", result_img.size, (255, 255, 255, 0))
                        temp_img.paste(overlay_img, position)
                        result_img = Image.blend(result_img, temp_img, 0.5)
                    
                    else:
                        # Default to normal for unsupported blend modes
                        if overlay_img.mode == "RGBA":
                            result_img.paste(overlay_img, position, overlay_img)
                        else:
                            result_img.paste(overlay_img, position)
                    ...
                    output_path = os.path.join(self.temp_dir, f"{base_name}_composite.png")
            ...
            logger.error(f"Image compositing failed: {str(e)}")
            return {
                "success": False,
                "error": str(e),
                "error_type": type(e).__name__,
